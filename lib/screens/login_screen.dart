...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // L'URL est construite dynamiquement depuis AppConfig
  // Assurez-vous que AppConfig.apiBaseUrl pointe vers la bonne adresse :
  // - Émulateur Android : http://10.0.2.2:3000/api
  // - Web (localhost) : http://localhost:3000/api
  // - Vrai téléphone : http://[IP_PC]:3000/api
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      // Si l'erreur contient des détails sur la connexion, les afficher
      String errorDetail = e.toString();
      if (errorDetail.contains('Connection refused') ||
          errorDetail.contains('Failed host lookup')) {
        _errorMessage =
            '❌ Impossible de joindre le serveur. Vérifiez que le backend tourne et que l\'URL est correcte.';
      } else {
        _errorMessage = '❌ Erreur : $errorDetail';
      }
      setState(() {
        _isLoading = false;
      });
